...
const tryRetrieval = () => {
  try {
    ...
    console.log("Retrieved article successfully");
    if (!article) throw new Error('Error getting article');
    program();
  } catch (error) {
    console.log(`${error}`);
    setTimeout(tryRetrieval, 500);
  }
};
  ...
  const loadJson = (callback) => {
    ...
    fetch(jsonUrl)
      ...
      .then((newContext) => {
        ...
        context = newContext;
        // console.log('loadJson',context, newContext);
        callback()
      })
      .catch((error) => {
